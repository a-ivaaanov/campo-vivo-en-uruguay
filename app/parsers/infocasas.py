...
class InfoCasasParser(BaseParser):
    ...
    def __init__(self, proxy_list: Optional[List[str]] = None):
        ...
        # Инициализируем множество для хранения глобально обработанных URL
    ...
    async def run(self, max_pages: Optional[int] = None, headless: bool = True) -> List[Listing]:
        ...
        try:
            ...
            result = await super().run(max_pages=max_pages, headless=headless)
            ...
            for i, listing in enumerate(listings):
                ...
                try:
                    ...
                    if detailed_listing:
                        ...
                        self.logger.warning(f"Не удалось получить детали для {listing.url}, используем оригинальное объявление")
